...
import { Point2D, Rectangle2D, Rectangle2DInterface, TaggedPoint2D } from "./graphics";
...
export class SiteModel {
  ...
  constructor(conditions: DesignConditions, drawingWindowCustomizer?: (drawingWindow: Rectangle2DInterface) => void) {
    ...
    while (SiteConstants.ELEVATION_TERRAIN_POINTS[this.leftAbutmentInterfaceTerrainIndex].x + this.halfCutGapWidth < leftLimit) {
    ...
    while (SiteConstants.ELEVATION_TERRAIN_POINTS[this.rightAbutmentInterfaceTerrainIndex].x + this.halfCutGapWidth > rightLimit) {
    ...
    this.drawingWindow.y = this.yGradeLevel - SiteConstants.WATER_BELOW_GRADE - 3.5;
    this.drawingWindow.height = this.yGradeLevel + SiteConstants.OVERHEAD_CLEARANCE + 1.0 - this.drawingWindow.y;
...
export class SiteConstants {
  ...
  static readonly BEAM_HEIGHT: number = 0.90;
  ...
  static readonly WEAR_SURFACE_HEIGHT: number = 0.80;
  ...
  static readonly ELEVATION_TERRAIN_POINTS: Point2D[] = ([
    ...
    [-this.FAR_AWAY, -this.FAR_AWAY],
    // #endregion
  ] as [number, number][]).map(pair => new Point2D(...pair));
  ...
  static readonly ARCH_ABUTMENT_POINTS: TaggedPoint2D<PointTag | undefined>[] = ([
    ...
    // #endregion
  ] as [number, number, PointTag | undefined][]).map(triple => new TaggedPoint2D<PointTag | undefined>(...triple));
  ...
  static readonly PIER_POINTS: TaggedPoint2D<PointTag | undefined>[] = ([
    ...
    [1.40, -7.5, PointTag.HEIGHT_ADJUSTED],
    [1.40, -8.0, PointTag.HEIGHT_ADJUSTED],
    [-1.40, -8.0, PointTag.HEIGHT_ADJUSTED],
    [-1.40, -7.5, PointTag.HEIGHT_ADJUSTED],
    [-0.75, -7.5, PointTag.HEIGHT_ADJUSTED],
    [-0.75, -0.2, PointTag.HEIGHT_ADJUSTED],
