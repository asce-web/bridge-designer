...
export class AboutDialogComponent implements AfterViewInit {
  ...
  handlePurposeButtonClick(): void {
    this.eventBrokerService.helpRequest.next({ origin: EventOrigin.ABOUT_DIALOG, data: 'hlp_purposes' });
  }

  handleHowItWorksButtonClick() {
    this.eventBrokerService.helpRequest.next({ origin: EventOrigin.ABOUT_DIALOG, data: 'hlp_how_wpbd_works' });
  }
