...
export class AddMemberCommand extends EditCommand {
  ...
  constructor(
    ...
    const context: SelectableBridge = { bridge, elementSelection };
    // Handle the most common case without copying.
    if (transecting.length === 0) {
      super(
        `Add member, joint ${member.a.number} to ${member.b.number}`,
        context
      );
    ...
    super(
      `Add ${transecting.length + 1} members, joint ${member.a.number} to ${
        member.b.number
      }`,
      context
    );
    ...
    this.members.push(
      new Member(-1, a, member.b, member.material, member.shape)
    );
  }
  ...
  public override do(): void {
    ...
    this.members.forEach(
      (member, index) => (member.index = index + bridge.members.length)
    );
