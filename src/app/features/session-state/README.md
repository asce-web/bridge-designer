...
at the app level to handle `window:beforeunload`. The handler writes state to local storage. Constructors reload state
previously saved, when it exists.
...
`BridgeService` injector - along with its state restoration - has already been populated.
...
A simple static token is also written to session (not local) storage when state is sent to local storage. Session
storage is associated with a browser tab and destroyed when it closes. If at state restoration time, the session storage
token is gone, we infer the user has closed the tab in the interim and show a dialog offering to continue or else reload
the app with empty state. A complete reload is necessary because we can't show an Angular dialog before state is already
restored...a chicken and egg problem. This gives a slightly janky user experience. The loading graphic re-displays during
a second or so delay, but alternatives all look worse.
