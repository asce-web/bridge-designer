...
export class MemberSplitter {
  ...
  constructor(pt: Point2DInterface, joint: Joint, members: Member[]) {
    ...
    for (const member of members) {
      ...
      if (Geometry.isPointOnSegment(pt, member.a, member.b) && !connectedMemberJointPairs.has(member.key)) {
        ...
        if (!connectedMemberJointPairs.has(Member.getJointsKey(member.a, joint))) {
          this.mergedMembers.push(new Member(index, member.a, joint, member.material, member.shape));
          index = -1;  // Second insert, if any, is un-indexed.
          existingMemberCount++;
        }
        if (!connectedMemberJointPairs.has(Member.getJointsKey(joint, member.b))) {
          this.mergedMembers.push(new Member(index, joint, member.b, member.material, member.shape));
        }
      }
    }
  ...
  public do(members: Member[]): void {
    if (members.length + this.mergedMembers.length - this.removedMembers.length > DesignConditions.MAX_MEMBER_COUNT) {
      throw new Error('max member count would be exceeded by splitting');
    }
    EditableUtility.remove(members, this.removedMembers);
    EditableUtility.merge(members, this.mergedMembers);
  }

  /** Undo member splitting in the given list. */
  public undo(members: Member[]):void {
    EditableUtility.remove(members, this.mergedMembers);
    EditableUtility.merge(members, this.removedMembers);
  }
}
