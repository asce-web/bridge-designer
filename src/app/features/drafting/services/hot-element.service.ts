...
export class HotElementService {
  ...
  public updateRenderedHotElementForViewportPoint(
    ctx: CanvasRenderingContext2D,
    x: number,
    y: number,
    considerOnly?: HotElementClass[]): void {
    ...
    if (!considerOnly || considerOnly.includes(Joint)) {
      const jointRadiusWorldSquared = Utility.sqr(3 * this.viewportTransform.viewportToWorldDistance(DesignJointRenderingService.JOINT_RADIUS_VIEWPORT));
    ...
    if (!considerOnly || considerOnly.includes(Member)) {
      bridge.members.forEach(member => {
        ...
        if (Geometry.isInNonAxisAlignedRectangle(xWorld, yWorld, member.a.x, member.a.y, member.b.x, member.b.y, width)) {
          hotElement = member;
